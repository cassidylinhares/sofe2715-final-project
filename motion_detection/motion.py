...
def convertToGrayscale(surfArr):
    ...
    surfArr[:, :, 0] = luminenceCalc(
        surfArr[:, :, 0], surfArr[:, :, 1], surfArr[:, :, 2])
    surfArr[:, :, 1] = luminenceCalc(
        surfArr[:, :, 0], surfArr[:, :, 1], surfArr[:, :, 2])
    surfArr[:, :, 2] = luminenceCalc(
        surfArr[:, :, 0], surfArr[:, :, 1], surfArr[:, :, 2])
...
while running:
    ...
    screen.fill((0, 0, 0))
    if (not isFirstFrame):
        frameDiff = abs(convertToGrayscale(previousFrame) -
                        convertToGrayscale(currentFrame))
